...
int my_getattr(const char *path, struct stat *statbuf)
{
    ...
    if (retstat != 0)
	retstat = my_error("my_getattr lstat");
...
int my_readlink(const char *path, char *link, size_t size)
{
    ...
    log_msg("my_readlink(path=\"%s\", link=\"%s\", size=%d)\n",
	  path, link, size);
    my_fullpath(fpath, path);
    
    retstat = readlink(fpath, link, size - 1);
    if (retstat < 0)
	retstat = my_error("my_readlink readlink");
    else  {
	link[retstat] = '\0';
	retstat = 0;
    }
...
int my_mknod(const char *path, mode_t mode, dev_t dev)
{
	...
	if (retstat < 0)
	    retstat = my_error("my_mknod open");
        else {
            retstat = close(retstat);
	    if (retstat < 0)
		retstat = my_error("my_mknod close");
	}
    } else
	if (S_ISFIFO(mode)) {
	    retstat = mkfifo(fpath, mode);
	    if (retstat < 0)
		retstat = my_error("my_mknod mkfifo");
	} else {
	    retstat = mknod(fpath, mode, dev);
	    if (retstat < 0)
		retstat = my_error("my_mknod mknod");
	}
...
int my_mkdir(const char *path, mode_t mode)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_mkdir mkdir");
...
int my_unlink(const char *path)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_unlink unlink");
...
int my_rmdir(const char *path)
{
    ...
    retstat = rmdir(fpath);
    if (retstat < 0)
	retstat = my_error("my_rmdir rmdir");
    
    return retstat;
}

/** Create a symbolic link */
//  The 'path' is where the link points,
...
int my_symlink(const char *path, const char *link)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_symlink symlink");
...
int my_rename(const char *path, const char *newpath)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_rename rename");
...
int my_chmod(const char *path, mode_t mode)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_chmod chmod");
  ...
  
{
    ...
    if (retstat < 0)
	retstat = my_error("my_chown chown");
...
int my_utime(const char *path, struct utimbuf *ubuf)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_utime utime");
	     ...
	     struct fuse_file_info *fi)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_write pwrite");
...
int my_statfs(const char *path, struct statvfs *statv)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_statfs statvfs");
...
int my_setxattr(const char *path, const char *name, const char *value, size_t size, int flags)
{
    ...
    log_msg("\nmy_setxattr(path=\"%s\", name=\"%s\", value=\"%s\", size=%d, flags=0x%08x)\n",
	    path, name, value, size, flags);
    my_fullpath(fpath, path);
    
    retstat = lsetxattr(fpath, name, value, size, flags);
    if (retstat < 0)
	retstat = my_error("my_setxattr lsetxattr");
...
int my_getxattr(const char *path, const char *name, char *value, size_t size)
{
    ...
    log_msg("\nmy_getxattr(path = \"%s\", name = \"%s\", value = 0x%08x, size = %d)\n",
	    path, name, value, size);
    my_fullpath(fpath, path);
    
    retstat = lgetxattr(fpath, name, value, size);
    if (retstat < 0)
	retstat = my_error("my_getxattr lgetxattr");
    else
	log_msg("    value = \"%s\"\n", value);
...
int my_listxattr(const char *path, char *list, size_t size)
{
    ...
    log_msg("my_listxattr(path=\"%s\", list=0x%08x, size=%d)\n",
	    path, list, size
	    );
    my_fullpath(fpath, path);
    ...
    if (retstat < 0)
	retstat = my_error("my_listxattr llistxattr");
...
int my_removexattr(const char *path, const char *name)
{
    ...
    log_msg("\nmy_removexattr(path=\"%s\", name=\"%s\")\n",
	    path, name);
    my_fullpath(fpath, path);
    
    retstat = lremovexattr(fpath, name);
    if (retstat < 0)
	retstat = my_error("my_removexattr lrmovexattr");
...
int my_opendir(const char *path, struct fuse_file_info *fi)
{
    ...
    dp = opendir(fpath);
    if (dp == NULL)
	retstat = my_error("my_opendir opendir");
	       ...
	       struct fuse_file_info *fi)
{
    ...
    // once again, no need for fullpath -- but note to cast fi->fh
    ...
    de = readdir(dp);
    if (de == 0) {
	retstat = my_error("my_readdir readdir");
	return retstat;
    }
	...
	if (filler(buf, de->d_name, NULL, 0) != 0) {
	    log_msg("    ERROR my_readdir filler:  buffer full");
	    return -ENOMEM;
	}
...
int my_access(const char *path, int mask)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_access access");
...
int my_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    ...
    log_msg("\nmy_create(path=\"%s\", mode=0%03o, fi=0x%08x)\n",
	    path, mode, fi);
    my_fullpath(fpath, path);
    
    fd = creat(fpath, mode);
    if (fd < 0)
	retstat = my_error("my_create creat");
...
int my_ftruncate(const char *path, off_t offset, struct fuse_file_info *fi)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_ftruncate ftruncate");
...
int my_fgetattr(const char *path, struct stat *statbuf, struct fuse_file_info *fi)
{
    ...
    if (retstat < 0)
	retstat = my_error("my_fgetattr fstat");
...
int main(int argc, char *argv[])
{
	...
	fprintf(stderr, "Running myFS as root opens unnacceptable security holes\n");
	return 1;
    }
